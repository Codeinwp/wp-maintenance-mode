	...
	class WPMaintenanceMode {
		...
		function admin_init() {
			...
			if ( is_multisite() && is_plugin_active_for_network( plugin_basename( __FILE__ ) ) ) {
				...
				if ( version_compare( $GLOBALS[ 'wp_version' ], '2.7alpha', '>' ) ) {
					add_action(
						'after_plugin_row_' . FB_WM_BASENAME, array(
							'WPMaintenanceMode_Settings',
							'add_config_form'
						), 10, 3
					);
		...
		public function check_role() {
			...
			if ( current_user_can( $role ) ) {
				...
				if ( current_user_can( $value[ 'role_frontend' ][ 0 ] ) ) {
					return TRUE;
				}
			}
		...
		public function add_link() {
			...
			?>
			<div id="footer">
				<p><a href="http://bueltge.de/"><?php _e( 'Plugin by:', FB_WM_TEXTDOMAIN ); ?>
						<img src="http://bueltge.de/favicon.ico" alt="bueltge.de" width="16" height="16" /></a>
					<?php
					switch ( $value[ 'theme' ] ) {
						case 2:
		...
		public function add_content() {
			...
			if ( !empty( $value[ 'text' ] ) ) {
				$value[ 'text' ] = wpautop( $value[ 'text' ] );
			} // apply_filters( 'the_content', $value['text'] );
