...
if ( ! class_exists( 'WP_Maintenance_Mode_Admin' ) ) {
	...
	class WP_Maintenance_Mode_Admin {
			...
			if ( $this->plugin_screen_hook_suffix === $screen->id ) {
				...
				wp_localize_script(
					...
					array(
						...
						'pluginURL'             => add_query_arg( array( 'page' => $this->plugin_slug ), wpmm_option_page_url() ),
						...
						'isNetworkSite'         => is_multisite() && is_network_admin(),
					)
				);
		...
		public function maybe_redirect() {
			...
			wp_safe_redirect( wpmm_option_page_url() );
			exit;
		}
				...
				if ( ! get_option( 'wpmm_fresh_install' ) && get_option( 'wpmm_new_look' ) && $this->plugin_settings['general']['status'] === 1 ) {
						...
						if ( ( $maintenance_page instanceof WP_Post ) && $maintenance_page->post_status !== 'publish' && $maintenance_page->post_status !== 'private' ) {
							$notices['maintenance_page_deleted'] = array(
								...
								'msg'   => $maintenance_page->post_status === 'draft' ?
									...
									sprintf( __( '<strong>Action required</strong>: your Maintenance page has been deleted. Visit <a href="%s">settings page</a> to address this issue.', 'wp-maintenance-mode' ), wpmm_option_page_url() . '#design' ),
							);
						}
					}
					...
					if ( ! get_post( $this->plugin_settings['design']['page_id'] ) ) {
						$notices['maintenance_page_not_found'] = array(
							'class' => 'error',
							'msg'   => sprintf( __( '<strong>Action required</strong>: you don\'t have a page as Maintenance page. Visit <a href="%s">settings page</a> to select one.', 'wp-maintenance-mode' ), wpmm_option_page_url() . '#design' ),
						);
					}
				}
